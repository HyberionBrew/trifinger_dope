...
# Configuration
algo_values = ["mb", "fqe", "dr", "iw"]
policy_class = "trifinger_rl_example.example.TorchPushPolicy"
std_values = ["0.05", "0.2"]
max_concurrent_processes = 2  # Set the number of concurrent processes here
...
def run_job(algo, std):
    ...
    command = (
        ...
        f"--target_policy_std={std} --num_mc_episodes=10 --nobootstrap --algo={algo} "
        f"--noise_scale=0.0 --num_updates={num_updates} --no_mc --discount=0.995 {extra_flags}"
    )
